...
class Scattering2dDenseNet(nn.Module):
    ...
    def build(self):
        ...
        self.dense.features = nn.Sequential(*list(self.dense.features[:4]) + list(self.dense.features[6:]))
        self.dense.features[0] = nn.Conv2d(self.scattering_output_channels, 128, kernel_size=7, stride=2, padding=3, bias=False)
        self.dense.features[1] = nn.BatchNorm2d(128, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
#         self.dense.features[5].denselayer1.norm1 = nn.BatchNorm2d(self.in_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
#         self.dense.features[0].denselayer1.norm2 = nn.BatchNorm2d(self.in_channels, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True)
#         self.dense.features[0].denselayer1.conv1 = nn.Conv2d(self.in_channels, 128, kernel_size=1, stride=1, bias=False)
#         self.dense.features[0].denselayer1.conv2 = nn.Conv2d(self.in_channels, 32, kernel_size=3, stride=1, padding=1, bias=False)
...
model = Scattering2dDenseNet(K).to(device)
...
for epoch in range(0, num_epoch):
    ...
    if (epoch + 1) % 20 == 0:
        torch.save(model.state_dict(), f'DenseNet_1st_block_removed_epoch_{epoch+1}.pth')
