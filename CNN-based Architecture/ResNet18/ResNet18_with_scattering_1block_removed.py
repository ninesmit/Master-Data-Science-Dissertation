...
class Scattering2dResNet(nn.Module):
    ...
    def build(self):
        ...
        self.resnet =  nn.Sequential(*list([self.resnet.layer2
                                            ,self.resnet.layer3
                                            ,self.resnet.layer4
                                            ,self.resnet.avgpool]))

        # Adjust the first layer of the first residual block to match the scattering output channels
        self.resnet[0][0].conv1 = nn.Conv2d(self.scattering_output_channels, 128, kernel_size=3, stride=2, padding=1, bias=False)
        self.resnet[0][0].downsample[0] = nn.Conv2d(self.scattering_output_channels, 128, kernel_size = 1, stride=2, bias=False)
...
for epoch in range(0, num_epoch):
    ...
    if (epoch + 1) % 20 == 0:
        torch.save(model.state_dict(), f'Resnet18_1st_block_removed_epoch_{epoch+1}.pth')
