...
class Scattering2dResNet(nn.Module):
    ...
    def build(self):
        ...
        self.resnet =  nn.Sequential(*list([self.resnet.layer3
                                            ,self.resnet.layer4
                                            ,self.resnet.avgpool]))

        # Adjust the first layer of the first residual block to match the scattering output channels
        self.resnet[0][0].conv1 = nn.Conv2d(self.scattering_output_channels, 256, kernel_size=3, stride=2, padding=1, bias=False)
        self.resnet[0][0].downsample[0] = nn.Conv2d(self.scattering_output_channels, 256, kernel_size = 1, stride=2, bias=False)
